...
func NewResolverCache(maxEntries int) *ResolverCache {
	...
	return &ResolverCache{maxEntries:maxEntries, items:make(map[string]*list.Element), lru:list.New()}
}

func (c *ResolverCache) Get(key string, now time.Time) ([]string, bool) {
	c.mu.Lock(); defer c.mu.Unlock()
	e, ok := c.items[key]; if !ok { return nil, false }
	entry := e.Value.(CacheEntry)
	if !entry.ExpiresAt.After(now) { delete(c.items,key); c.lru.Remove(e); return nil,false }
	c.lru.MoveToFront(e)
...
func (c *ResolverCache) PurgeExpired(now time.Time) int {
	...
	for e:=c.lru.Back(); e!=nil; { prev:=e.Prev(); entry:=e.Value.(CacheEntry); if !entry.ExpiresAt.After(now) { delete(c.items,entry.Key); c.lru.Remove(e); removed++ }; e=prev }
	return removed
}
