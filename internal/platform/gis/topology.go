...
func (g *TopologyGraph) Upsert(l TopologyLink) { g.mu.Lock(); g.links[l.ID] = l; g.mu.Unlock() }

func (g *TopologyGraph) List() []TopologyLink {
	g.mu.RLock(); defer g.mu.RUnlock()
	out := make([]TopologyLink, 0, len(g.links))
	for _, l := range g.links { out = append(out, l) }
	return out
}
...
func (g *TopologyGraph) Neighbors(id string) []TopologyLink {
	...
	for _, l := range g.links { if l.FromID == id || l.ToID == id { out = append(out, l) } }
	return out
}
