...
func(g *TopologyGraph)Upsert(l TopologyLink){if g==nil{return};l.ID=strings.TrimSpace(l.ID);l.FromID=strings.TrimSpace(l.FromID);l.ToID=strings.TrimSpace(l.ToID);l.Kind=strings.TrimSpace(l.Kind);l.Status=strings.ToLower(strings.TrimSpace(l.Status));if l.ID==""||l.FromID==""||l.ToID==""||l.FromID==l.ToID{return};g.mu.Lock();defer g.mu.Unlock();if g.links==nil{g.links=make(map[string]TopologyLink)};g.links[l.ID]=l}
