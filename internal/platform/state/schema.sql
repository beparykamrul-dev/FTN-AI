...
CREATE TABLE IF NOT EXISTS ftn_route_decisions (
    ...
    mutation_blocked BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_ftn_audit_events_created_at ON ftn_audit_events(created_at);
CREATE INDEX IF NOT EXISTS idx_ftn_route_decisions_service ON ftn_route_decisions(service_id, created_at);
