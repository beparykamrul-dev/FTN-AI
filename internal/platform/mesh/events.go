...
func(b *EventBus)Publish(e Event){if b==nil{return};e.NodeID=strings.TrimSpace(e.NodeID);if e.At.IsZero(){e.At=time.Now().UTC()}else{e.At=e.At.UTC()};if e.Type!=""{e.Type=EventType(strings.TrimSpace(string(e.Type)))};b.mu.RLock();defer b.mu.RUnlock();for ch:=range b.subs{select{case ch<-e:default:}}}
