...
type WorkflowStep struct { ID string; ToolID string; Input map[string]any; Required bool }
...
type ToolExecutor interface { Execute(ctx context.Context, step WorkflowStep) error }
...
func (e *Engine) Create(w Workflow) error {
	...
	if w.ID == "" || len(w.Steps) == 0 { return errors.New("workflow id and steps are required") }
	for i := range w.Steps {
		w.Steps[i].ID = strings.TrimSpace(w.Steps[i].ID)
		w.Steps[i].ToolID = strings.TrimSpace(w.Steps[i].ToolID)
		if w.Steps[i].ID == "" || w.Steps[i].ToolID == "" { return errors.New("workflow steps require id and tool id") }
	}
...
func (e *Engine) Resume(ctx context.Context, id string) error {
	...
	if id == "" { return ErrWorkflowNotFound }
	for {
		select { case <-ctx.Done(): return ctx.Err(); default: }
		e.mu.Lock(); w, ok := e.workflows[id]
		if !ok { e.mu.Unlock(); return ErrWorkflowNotFound }
		if w.State == WorkflowSucceeded || w.State == WorkflowCancelled { e.mu.Unlock(); return ErrWorkflowNotRunnable }
		w.State = WorkflowRunning; w.UpdatedAt = time.Now().UTC()
		if w.CurrentStep >= len(w.Steps) { w.State = WorkflowSucceeded; e.workflows[id] = cloneWorkflow(w); e.mu.Unlock(); return nil }
