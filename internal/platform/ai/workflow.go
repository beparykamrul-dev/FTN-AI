...
// ToolExecutor is the narrow execution boundary. Implementations must perform
// authentication, capability, policy, risk and approval checks before mutation.
type ToolExecutor interface {
	Execute(ctx context.Context, step WorkflowStep) error
}
...
func NewEngine(executor ToolExecutor) *Engine {
	return &Engine{workflows: make(map[string]Workflow), executor: executor}
}

func (e *Engine) Create(w Workflow) error {
	if w.ID == "" || len(w.Steps) == 0 {
		return errors.New("workflow id and steps are required")
	}
	if w.State == "" {
		w.State = WorkflowPending
	}
...
func (e *Engine) Resume(ctx context.Context, id string) error {
	...
	for {
		...
		if w.CurrentStep >= len(w.Steps) {
			...
			e.workflows[id] = cloneWorkflow(w)
			e.mu.Unlock()
			return nil
		}
		...
		if err := e.executor.Execute(ctx, step); err != nil {
			...
			w.LastError = fmt.Sprintf("step %s: %v", step.ID, err)
			w.Revision++
			w.UpdatedAt = time.Now().UTC()
			e.workflows[id] = cloneWorkflow(w)
			e.mu.Unlock()
		...
		e.workflows[id] = cloneWorkflow(w)
		done := w.State == WorkflowSucceeded
		e.mu.Unlock()
		if done {
			return nil
		}
...
func cloneWorkflow(w Workflow) Workflow {
	...
	for i := range out.Steps {
		out.Steps[i].Input = cloneMap(w.Steps[i].Input)
	}
	return out
}
