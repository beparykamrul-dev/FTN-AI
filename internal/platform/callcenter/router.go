...
func(r *Router)Register(eventType string,h Handler){if r==nil||h==nil{return};eventType=strings.TrimSpace(eventType);if eventType==""{return};r.mu.Lock();defer r.mu.Unlock();if r.handlers==nil{r.handlers=make(map[string][]Handler)};for _,existing:=range r.handlers[eventType]{if &existing==&h{return}};r.handlers[eventType]=append(r.handlers[eventType],h)}
