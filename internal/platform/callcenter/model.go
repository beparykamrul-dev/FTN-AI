...
func(e CallEvent)Valid()bool{sid:=strings.TrimSpace(e.SessionID);typ:=strings.TrimSpace(e.Type);return sid!=""&&len(sid)<=256&&typ!=""&&len(typ)<=128&&!e.Timestamp.IsZero()&&len(e.Payload)<=128}
