...
func(s CallSession)Valid()bool{status:=strings.ToLower(strings.TrimSpace(s.Status));return strings.TrimSpace(s.ID)!=""&&strings.TrimSpace(s.CustomerID)!=""&&(s.Channel==ChannelVoice||s.Channel==ChannelWeb||s.Channel==ChannelAndroid)&&!s.StartedAt.IsZero()&&status!=""}
func(c CallCenterConfig)Valid()bool{if c.MaxSessionMinutes<=0{return false};if c.Policy.CanExecuteNetworkAction&&!c.Policy.RequiresApproval{return false};return true}
