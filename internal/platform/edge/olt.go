...
func(s OLTState)Valid()bool{id:=strings.TrimSpace(s.ID);vendor:=strings.TrimSpace(s.Vendor);model:=strings.TrimSpace(s.Model);status:=strings.TrimSpace(s.Status);ip:=net.ParseIP(strings.TrimSpace(s.ManagementIP));return id!=""&&vendor!=""&&len(id)<=256&&len(vendor)<=128&&len(model)<=256&&len(status)<=64&&ip!=nil&&!ip.IsUnspecified()}
