...
// ClientPlatform is the canonical client platform type for edge enrollment
...
func ValidateClientEnrollment(e ClientEnrollment) error {
	if e.DeviceID == "" || e.UserID == "" || e.NodeID == "" {
		return fmt.Errorf("device enrollment is incomplete")
	}
	switch e.Platform {
	case PlatformPC, PlatformTV, PlatformAndroid, PlatformWindows, PlatformLinux, PlatformMacOS:
	default:
		return fmt.Errorf("unsupported client platform")
	}
	if e.CertificateID == "" {
