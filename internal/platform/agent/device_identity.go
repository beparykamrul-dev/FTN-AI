...
func(d DeviceIdentity)Normalized()DeviceIdentity{d.ID=strings.TrimSpace(d.ID);d.Name=strings.TrimSpace(d.Name);d.Hostname=strings.TrimSpace(d.Hostname);d.IP=strings.TrimSpace(d.IP);if d.IP!=""&&net.ParseIP(d.IP)==nil{d.IP=""};d.MAC=strings.ToLower(strings.TrimSpace(d.MAC));d.Serial=strings.TrimSpace(d.Serial);d.OS=strings.TrimSpace(d.OS);d.AgentID=strings.TrimSpace(d.AgentID);if !d.LastSeen.IsZero(){d.LastSeen=d.LastSeen.UTC()};return d}
