...
func BuildSummary(items []SummaryItem) Summary {
	...
	sort.SliceStable(important, func(i, j int) bool { return important[i].Priority > important[j].Priority })
	sort.SliceStable(details, func(i, j int) bool { return important[i].Priority > important[j].Priority })
	if len(important) > 5 { important = important[:5] }
