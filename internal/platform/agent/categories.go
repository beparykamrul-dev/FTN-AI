...
func(c CategoryConfig)Valid()bool{scope:=strings.TrimSpace(c.ToolScope);id:=strings.TrimSpace(string(c.ID));if id==""||scope==""||len(scope)>128||strings.ContainsAny(scope,"\r\n\x00"){return false};switch c.ID{case CategoryStudio,CategoryCallCenter,CategoryBilling,CategoryNetwork,CategoryDeveloper,CategoryCustomer,CategoryExecutive:return true;default:return false}}
