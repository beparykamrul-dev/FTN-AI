...
func(c CategoryConfig)Valid()bool{scope:=strings.TrimSpace(c.ToolScope);if strings.TrimSpace(string(c.ID))==""||scope==""||len(scope)>128{return false};switch c.ID{case CategoryStudio,CategoryCallCenter,CategoryBilling,CategoryNetwork,CategoryDeveloper,CategoryCustomer,CategoryExecutive:return true;default:return false}}
