...
// Run executes the active DNS probe loop and converts query observations to
// the canonical DNSProbeResult consumed by anycast policy/orchestration.
func (s *AnycastScheduler) Run(ctx context.Context, schedule AnycastSchedule, reconcile func(context.Context, DNSProbeResult) error) error {
	...
	defer ticker.Stop()
	run := func() error {
		q := schedule.Probe.Probe(ctx)
		result := DNSProbeResult{Address: schedule.Probe.Address, Reachable: q.Reachable && q.Answered, Latency: q.Latency, Error: q.Error}
