...
func (r DMARCRecord) Validate() error {
	...
	switch r.Policy { case DMARCNone, DMARCQuarantine, DMARCReject: default: return fmt.Errorf("invalid DMARC policy") }
	if r.SubdomainPolicy != "" { switch r.SubdomainPolicy { case DMARCNone, DMARCQuarantine, DMARCReject: default: return fmt.Errorf("invalid subdomain policy") } }
	if r.Percentage > 100 { return fmt.Errorf("percentage must be between 0 and 100") }
	return nil
}
...
func (r DMARCRecord) TXTValue() (string, error) {
	...
	if r.SubdomainPolicy != "" { parts = append(parts, "sp="+string(r.SubdomainPolicy)) }
	parts = append(parts, fmt.Sprintf("pct=%d", r.Percentage))
	if len(r.AggregateReportURI) > 0 { parts = append(parts, "rua="+strings.Join(r.AggregateReportURI, ",")) }
	if len(r.ForensicReportURI) > 0 { parts = append(parts, "ruf="+strings.Join(r.ForensicReportURI, ",")) }
