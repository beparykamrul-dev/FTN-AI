...
func (r SPFRecord) Validate() error {
	...
	if r.All != "" && r.All != "pass" && r.All != "fail" && r.All != "softfail" && r.All != "neutral" {
		return fmt.Errorf("invalid all qualifier")
	}
...
func (r SPFRecord) TXTValue() (string, error) {
	...
	for _, ip := range r.IP4 {
		parts = append(parts, "ip4:"+ip)
	}
	for _, ip := range r.IP6 {
		parts = append(parts, "ip6:"+ip)
	}
	for _, inc := range r.Include {
		if strings.TrimSpace(inc) != "" {
			parts = append(parts, "include:"+strings.TrimSpace(inc))
		}
	}
	if r.All != "" {
