...
func (c *ProbePolicyController) Observe(ctx context.Context, key string, result DNSProbeResult, policy ProbePolicy) (bool, error) {
	if c == nil { return false, fmt.Errorf("probe policy controller is required") }
	if err := policy.Validate(); err != nil { return false, err }
	if ctx == nil { return false, context.Canceled }
	if err := ctx.Err(); err != nil { return false, err }
	key = strings.TrimSpace(key)
	if key == "" { return false, fmt.Errorf("probe policy key is required") }
	c.mu.Lock()
	defer c.mu.Unlock()
	state := c.states[key]
	if result.Reachable && result.Latency <= policy.MaxLatency {
		state.Failures = 0
		if state.Successes < ^uint32(0) { state.Successes++ }
		if state.Successes >= policy.SuccessThreshold { state.Healthy = true }
	} else {
		state.Successes = 0
		if state.Failures < ^uint32(0) { state.Failures++ }
