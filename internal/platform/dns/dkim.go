...
func (r DKIMRecord) Validate() error {
	...
	if r.KeyType != "" && r.KeyType != "rsa" && r.KeyType != "ed25519" {
		return fmt.Errorf("unsupported DKIM key type")
	}
	if r.Hash != "" && r.Hash != "sha256" {
		return fmt.Errorf("unsupported DKIM hash")
	}
	return nil
}
