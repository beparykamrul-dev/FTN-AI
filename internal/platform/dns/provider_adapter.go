...
func NormalizeProviderEndpoint(endpoint string)string{endpoint=strings.TrimRight(strings.TrimSpace(endpoint),"/");if endpoint==""{return ""};u,err:=url.Parse(endpoint);if err!=nil||u.Scheme==""||u.Host==""{return endpoint};u.Path=strings.TrimRight(u.Path,"/");return strings.TrimRight(u.String(),"/")}
