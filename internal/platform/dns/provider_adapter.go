...
func(r *AdapterRegistry)Register(adapter ProviderAdapter)error{if r==nil{return fmt.Errorf("adapter registry is required")};if adapter==nil{return fmt.Errorf("adapter is required")};typ:=adapter.Type();if strings.TrimSpace(string(typ))==""{return fmt.Errorf("adapter type is required")};r.mu.Lock();defer r.mu.Unlock();if r.adapters==nil{r.adapters=make(map[ProviderType]ProviderAdapter)};if _,exists:=r.adapters[typ];exists{return fmt.Errorf("adapter already registered: %s",typ)};r.adapters[typ]=adapter;return nil}
func(r *AdapterRegistry)Get(t ProviderType)(ProviderAdapter,bool){if r==nil{return nil,false};r.mu.RLock();a,ok:=r.adapters[t];r.mu.RUnlock();return a,ok}
