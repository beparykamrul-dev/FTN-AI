...
// RouterOSCredentialRef identifies a secret-store entry. Plaintext credentials
// must not be persisted in device models, job payloads, or repository config.
type RouterOSCredentialRef string

type RouterOSSecret struct {
	Username string
	Password string
}
...
func NewRouterOSAdapter(baseURL string, credentialRef RouterOSCredentialRef, serverName string, resolveSecret func(context.Context, RouterOSCredentialRef) (RouterOSSecret, error), timeout time.Duration) (*RouterOSAdapter, error) {
	...
	return &RouterOSAdapter{
		baseURL: baseURL, credentialRef: credentialRef, serverName: serverName, resolveSecret: resolveSecret,
		client: &http.Client{Timeout: timeout, Transport: &http.Transport{TLSClientConfig: &tls.Config{MinVersion: tls.VersionTLS12, ServerName: serverName}}},
	}, nil
}
...
func (a *RouterOSAdapter) doJSON(ctx context.Context, path string) ([]map[string]any, error) {
	secret, err := a.resolveSecret(ctx, a.credentialRef)
	if err != nil { return nil, fmt.Errorf("routeros secret lookup: %w", err) }
...
func (a *RouterOSAdapter) CollectInterfaceState(ctx context.Context, device NetworkDevice) ([]InterfaceState, error) {
	...
	for _, row := range rows {
		name := stringValue(row["name"]); if name == "" { continue }
		out = append(out, InterfaceState{DeviceID: device.ID, Name: name, AdminUp: !boolValue(row["disabled"]), OperUp: boolValue(row["running"]), RxBps: uint64Value(row["rx-byte"]), TxBps: uint64Value(row["tx-byte"]), RxErrors: uint64Value(row["rx-error"]), TxErrors: uint64Value(row["tx-error"])})
	}
	return out, nil
}
...
func (a *RouterOSAdapter) CollectRoutingState(ctx context.Context, device NetworkDevice) ([]RoutingState, error) {
	...
	for _, row := range rows {
		prefix := stringValue(row["dst-address"]); if prefix == "" { continue }
		out = append(out, RoutingState{DeviceID: device.ID, Protocol: stringValue(row["protocol"]), VRF: stringValue(row["routing-table"]), Prefix: prefix, NextHop: stringValue(row["gateway"]), Metric: uint32Value(row["distance"]), Active: boolValue(row["active"])})
	}
	return out, nil
}
...
func uint64Value(v any) uint64 {
	switch n := v.(type) {
	case float64: if n > 0 { return uint64(n) }
	case json.Number: if value, err := n.Int64(); err == nil && value > 0 { return uint64(value) }
	case string: var value uint64; if _, err := fmt.Sscan(n, &value); err == nil { return value }
	}
	return 0
}
