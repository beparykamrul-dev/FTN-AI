...
func TestDiffRouterOSQoSStateNoOp(t *testing.T) {
	...
	diff, err := DiffRouterOSQoSState(RouterOSSnapshot{DeviceID: "r1", Rules: []RouterOSQoSState{rule}, CapturedAt: now}, RouterOSDesiredState{DeviceID: "r1", Rules: []RouterOSQoSState{rule}})
...
func TestDiffRouterOSQoSStateAddChangeRemove(t *testing.T) {
	...
	diff, err := DiffRouterOSQoSState(RouterOSSnapshot{DeviceID: "r1", Rules: []RouterOSQoSState{before, change}}, RouterOSDesiredState{DeviceID: "r1", Rules: []RouterOSQoSState{after, qosRule("imo", "path-z", "voice", 46, 92)}})
	if err != nil { t.Fatal(err) }
...
func TestNormalizeRouterOSQoSStateDeterministic(t *testing.T) {
	rules, err := NormalizeRouterOSQoSState([]RouterOSQoSState{qosRule("telegram", "path-z", "realtime", 46, 90), qosRule("whatsapp", "path-a", "voice", 46, 90)})
	if err != nil { t.Fatal(err) }
...
func TestNormalizeRouterOSQoSStateRejectsInvalidInput(t *testing.T) {
	cases := []RouterOSQoSState{{ServiceID: "", PathID: "path-a"}, {ServiceID: "whatsapp", PathID: ""}, {ServiceID: "whatsapp", PathID: "path-a", DSCP: 64}, {ServiceID: "not-a-managed-service", PathID: "path-a"}}
...
func TestDiffRouterOSQoSStateRejectsDeviceMismatch(t *testing.T) {
	...
	if _, err := DiffRouterOSQoSState(RouterOSSnapshot{DeviceID: "r1", Rules: []RouterOSQoSState{rule}}, RouterOSDesiredState{DeviceID: "r2", Rules: []RouterOSQoSState{rule}}); err == nil { t.Fatal("expected device mismatch error") }
}
...
func TestBuildRouterOSQoSExecutionPlanNeverAllowsApply(t *testing.T) {
	rule := qosRule("whatsapp", "path-a", "voice", 46, 90)
	plan, err := BuildRouterOSQoSExecutionPlan(RouterOSSnapshot{DeviceID: "r1"}, RouterOSDesiredState{DeviceID: "r1", Rules: []RouterOSQoSState{rule}})
	if err != nil { t.Fatal(err) }
	if !plan.RequiresApproval || plan.ApplyAllowed { t.Fatalf("unsafe execution plan: %#v", plan) }
}

func TestBuildRouterOSQoSExecutionPlanNoOp(t *testing.T) {
	rule := qosRule("whatsapp", "path-a", "voice", 46, 90)
	plan, err := BuildRouterOSQoSExecutionPlan(RouterOSSnapshot{DeviceID: "r1", Rules: []RouterOSQoSState{rule}}, RouterOSDesiredState{DeviceID: "r1", Rules: []RouterOSQoSState{rule}})
	if err != nil { t.Fatal(err) }
	if !plan.Diff.Empty() || plan.RequiresApproval || plan.ApplyAllowed { t.Fatalf("unexpected no-op plan: %#v", plan) }
}
