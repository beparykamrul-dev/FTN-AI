...
func (a *App) appendEvent(w http.ResponseWriter,r *http.Request){
	...
	rc:=requestInfo(r);if e.CorrelationID==""{e.CorrelationID=rc.CorrelationID};if e.Payload==nil{e.Payload=json.RawMessage(`{}`)};if a.db==nil{jsonResponse(w,503,map[string]string{"error":"database_required"});return}
	...
	if _,err=tx.Exec(r.Context(),`select pg_advisory_xact_lock(hashtext($1::text))`,rc.TenantID);err!=nil{jsonResponse(w,500,map[string]string{"error":"event_lock_failed"});return}
	var out DurableEvent
	err=tx.QueryRow(r.Context(),`insert into event_journal(tenant_id,event_type,sequence,correlation_id,causation_id,aggregate_id,payload) values(nullif($1,'')::uuid,$2,coalesce((select max(sequence)+1 from event_journal where tenant_id=nullif($1,'')::uuid),1),$3,$4,$5,$6::jsonb) returning id::text,coalesce(tenant_id::text,''),event_type,sequence,correlation_id,causation_id,aggregate_id,payload::text,created_at::text`,rc.TenantID,e.Type,e.CorrelationID,e.CausationID,e.AggregateID,string(e.Payload)).Scan(&out.ID,&out.TenantID,&out.Type,&out.Sequence,&out.CorrelationID,&out.CausationID,&out.AggregateID,&out.Payload,&out.CreatedAt)
	if err!=nil{a.audit(r,"event.append",e.Type,"failed",map[string]string{"error":"persist_failed"});jsonResponse(w,500,map[string]string{"error":"event_append_failed"});return};if err=tx.Commit(r.Context());err!=nil{jsonResponse(w,500,map[string]string{"error":"event_commit_failed"});return};a.audit(r,"event.append",out.Type,"accepted",out);jsonResponse(w,202,out)
}
