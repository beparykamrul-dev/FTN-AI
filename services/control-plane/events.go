...
func decodeEventBody(w http.ResponseWriter, r *http.Request, dst any) error {
    ...
    var extra any
    if err := dec.Decode(&extra); !errors.Is(err, io.EOF) { return errors.New("multiple_json_values") }
    return nil
}
...
func appendEventTx(tx pgx.Tx, ctx context.Context, tenantID, eventType, correlationID, causationID, aggregateID string, payload json.RawMessage) (DurableEvent, error) {
    if strings.TrimSpace(tenantID)=="" { return DurableEvent{}, errors.New("tenant_required") }
...
func (a *App) readEvents(w http.ResponseWriter,r *http.Request){
    if !method(w,r,http.MethodGet)||!requirePermission(a,"event.read",w,r){return};if a.db==nil{jsonResponse(w,503,map[string]string{"error":"database_required"});return};rc:=requestInfo(r);after,err:=strconv.ParseInt(r.URL.Query().Get("after"),10,64);if err!=nil&&r.URL.Query().Get("after")!=""{jsonResponse(w,400,map[string]string{"error":"invalid_after"});return};if after<0{jsonResponse(w,400,map[string]string{"error":"invalid_after"});return};limit,err:=strconv.Atoi(r.URL.Query().Get("limit"));if err!=nil&&r.URL.Query().Get("limit")!=""{jsonResponse(w,400,map[string]string{"error":"invalid_limit"});return};if limit<=0||limit>500{limit=100}
