...
func (a *App) verifyJob(w http.ResponseWriter, r *http.Request) {
	...
	if err:=json.NewDecoder(http.MaxBytesReader(w,r.Body,256<<10)).Decode(&in);err!=nil||strings.TrimSpace(in.WorkerID)==""||in.AttemptNo<=0{jsonResponse(w,400,map[string]string{"error":"invalid_verification"});return};if len(in.Result)==0{in.Result=json.RawMessage(`{}`)};if !json.Valid(in.Result){jsonResponse(w,400,map[string]string{"error":"invalid_result"});return}
	...
	res,err:=tx.Exec(r.Context(),`update durable_jobs set verification_payload=$2::jsonb,verified_by=$3::uuid,last_error='',updated_at=now() where id=$1::uuid and tenant_id=$4::uuid and verified_by is null`,id,string(in.Result),rc.PrincipalID,rc.TenantID);if err!=nil{jsonResponse(w,500,map[string]string{"error":"verification_update_failed"});return};if res.RowsAffected()!=1{jsonResponse(w,409,map[string]string{"error":"verification_already_recorded"});return}
