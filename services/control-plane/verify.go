...
func (a *App) verifyJob(w http.ResponseWriter, r *http.Request) {
	...
	if approvalID!=nil{var approvalStatus string;var approvalExpires interface{};err=tx.QueryRow(r.Context(),`select status,expires_at from change_approvals where id=$1::uuid and tenant_id=$2::uuid for update`,*approvalID,rc.TenantID).Scan(&approvalStatus,&approvalExpires);if err!=nil{jsonResponse(w,500,map[string]string{"error":"approval_state_query_failed"});return};if approvalStatus!="approved"{jsonResponse(w,409,map[string]string{"error":"approval_not_approved"});return}}
