...
func (a *App) verifyJob(w http.ResponseWriter, r *http.Request) {
    ...
    var status string;var attemptWorker,attemptStatus string;var approvalID *string
    ...
    if !in.Success{if _,err=tx.Exec(r.Context(),`update durable_jobs set verification_payload=$2::jsonb,last_error=$3,updated_at=now() where id=$1::uuid`,id,string(in.Result),in.Error);err!=nil{jsonResponse(w,500,map[string]string{"error":"verification_update_failed"});return}} else {if _,err=tx.Exec(r.Context(),`update durable_jobs set verification_payload=$2::jsonb,last_error='',updated_at=now() where id=$1::uuid`,id,string(in.Result));err!=nil{jsonResponse(w,500,map[string]string{"error":"verification_update_failed"});return};if approvalID!=nil{if _,err=tx.Exec(r.Context(),`update change_approvals set status='executed',executed_at=now(),updated_at=now() where id=$1::uuid and tenant_id=$2::uuid and status='approved'`,*approvalID,rc.TenantID);err!=nil{jsonResponse(w,500,map[string]string{"error":"approval_execution_update_failed"});return}}}
