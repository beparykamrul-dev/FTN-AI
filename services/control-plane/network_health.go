...
func (a *App) networkHealth(w http.ResponseWriter, r *http.Request) {
	if !method(w, r, http.MethodGet) || !requirePermission(a, "network.read", w, r) {
		return
	}
	rc := requestInfo(r)
	...
	for _, n := range ns {
		paths = append(paths, pathHealthFromNode(n))
	}
	sort.SliceStable(paths, func(i, j int) bool {
		if paths[i].Healthy != paths[j].Healthy {
			return paths[i].Healthy
		}
