...
// SetSiLKStore attaches the durable normalized-flow sink. Keeping this explicit
// prevents TrafficRuntime from inventing a database connection or tenant scope.
...
func (t *TrafficRuntime) Ingest(flows []FlowRecord,now time.Time)int{classified:=make([]TrafficFlowObservation,0,len(flows));for _,f:=range flows{if obs,ok:=t.Classify(f,now);ok{classified=append(classified,obs)}};if len(classified)==0{return 0};t.mu.Lock();silk:=t.silk;t.flows=append(t.flows,classified...);cutoff:=now.Add(-2*time.Minute);keep:=t.flows[:0];for _,f:=range t.flows{if f.ObservedAt.After(cutoff){keep=append(keep,f)}};t.flows=keep;if len(t.flows)>4096{t.flows=t.flows[len(t.flows)-4096:]};t.mu.Unlock();if silk!=nil{records:=make([]FlowRecord,0,len(classified));for _,obs:=range classified{records=append(records,obs.FlowRecord)};if err:=silk.InsertBatch(context.Background(),now,records,"","","");err!=nil{log.Printf("FTN SiLK flow persistence failed: %v",err)}};return len(classified)}
