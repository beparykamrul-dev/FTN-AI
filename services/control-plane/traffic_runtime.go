...
func (t *TrafficRuntime) UpsertEndpoint(e ManagedEndpoint) error {
    ...
    if len(t.endpoints) >= 4096 {
        return errors.New("endpoint_registry_limit")
    }
...
func (t *TrafficRuntime) Classify(f FlowRecord, now time.Time) (TrafficFlowObservation, bool) {
    ...
    for i := range endpoints {
        e := &endpoints[i]
        if e.network == nil || (!e.ExpiresAt.IsZero() && !now.Before(e.ExpiresAt)) {
            continue
        }
...
func (t *TrafficRuntime) Ingest(flows []FlowRecord, now time.Time) int {
    ...
    for _, f := range flows {
        if obs, ok := t.Classify(f, now); ok {
            classified = append(classified, obs)
        }
    }
    if len(classified) == 0 {
        return 0
    ...
    for _, f := range t.flows {
        if f.ObservedAt.After(cutoff) {
            keep = append(keep, f)
        }
    }
    t.flows = keep
    if len(t.flows) > 4096 {
        t.flows = t.flows[len(t.flows)-4096:]
    }
...
func (t *TrafficRuntime) Decisions(now time.Time, nodes []Node) []TrafficDecision {
    ...
    for serviceID, controller := range t.controllers {
    ...
    for serviceID, observations := range byService {
        ...
        if c == nil {
            ...
            t.mu.Lock()
            if existing := t.controllers[serviceID]; existing != nil {
                c = existing
            } else {
                t.controllers[serviceID] = c
            }
            t.mu.Unlock()
        }
