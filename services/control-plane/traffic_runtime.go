...
// Ingest classifies flows, retains a bounded in-memory window, then persists
// each service class as its own batch so the durable and analytical stores keep
// the classification metadata attached to the corresponding records.
func (t *TrafficRuntime) Ingest(flows []FlowRecord,now time.Time)int{
    ...
    t.mu.Lock();silk:=t.silk;t.flows=append(t.flows,classified...);cutoff:=now.Add(-2*time.Minute);keep:=t.flows[:0];for _,f:=range t.flows{if f.ObservedAt.After(cutoff){keep=append(keep,f)}};t.flows=keep;if len(t.flows)>4096{t.flows=t.flows[len(t.flows)-4096:]};t.mu.Unlock()
    if silk!=nil{
        byService:=make(map[string][]FlowRecord)
        for _,obs:=range classified{byService[obs.ServiceID]=append(byService[obs.ServiceID],obs.FlowRecord)}
        for serviceID,records:=range byService{
            if err:=silk.InsertBatch(context.Background(),now,records,serviceID,"","");err!=nil{log.Printf("FTN SiLK flow persistence failed for service %s: %v",serviceID,err)}
        }
    }
