...
type TrafficRuntime struct {
    ...
    activeProbe *TrafficActiveProbe
}
...
func (t *TrafficRuntime) StartActiveProbes(ctx context.Context, targets func(context.Context) []TrafficProbeTarget) error {
    ...
    probe := NewTrafficActiveProbe(t, interval, timeout)
    if err := probe.Start(ctx, func() []TrafficProbeTarget { return targets(ctx) }); err != nil {
        return err
    }
    t.mu.Lock()
    t.activeProbe = probe
...
func (t *TrafficRuntime) Close() error {
    ...
    var first error
    if probe != nil {
        if err := probe.Close(); err != nil { first = err }
    }
    if listener != nil {
        if err := listener.Close(); first == nil { first = err }
    }
    return first
}
...
func durationFromEnv(name string, fallback time.Duration) time.Duration {
    ...
    if err != nil || d <= 0 { return fallback }
    return d
}
...
func (t *TrafficRuntime) UpsertQuality(o TrafficQualityObservation, now time.Time) error {
    if t.quality == nil { return errors.New("traffic_quality_store_required") }
...
func (t *TrafficRuntime) UpsertEndpoint(e ManagedEndpoint) error {
    ...
    if e.ServiceID == "" || e.CIDR == "" { return errors.New("service_id_and_cidr_required") }
    if _, ok := trafficPolicyByID(e.ServiceID); !ok { return errors.New("unknown_service_id") }
    ...
    for i := range t.endpoints {
        if t.endpoints[i].ServiceID == e.ServiceID && t.endpoints[i].CIDR == e.CIDR { t.endpoints[i] = e; return nil }
    }
    if len(t.endpoints) >= 4096 { return errors.New("endpoint_registry_limit") }
...
func (t *TrafficRuntime) Classify(f FlowRecord, now time.Time) (TrafficFlowObservation, bool) {
    ...
    for i := range endpoints {
        e := &endpoints[i]
        if e.network == nil || (!e.ExpiresAt.IsZero() && !now.Before(e.ExpiresAt)) { continue }
        matched := (dst != nil && e.network.Contains(dst)) || (src != nil && e.network.Contains(src))
        if !matched { continue }
...
func trafficPolicyByID(id string) (TrafficServicePolicy, bool) {
    ...
    for _, p := range DefaultTrafficServicePolicies() { if p.ID == id { return p, true } }
    return TrafficServicePolicy{}, false
}

func (t *TrafficRuntime) Ingest(flows []FlowRecord, now time.Time) int {
    classified := make([]TrafficFlowObservation, 0, len(flows))
    for _, f := range flows { if obs, ok := t.Classify(f, now); ok { classified = append(classified, obs) } }
    if len(classified) == 0 { return 0 }
    t.mu.Lock()
    defer t.mu.Unlock()
    t.flows = append(t.flows, classified...)
...
func (t *TrafficRuntime) Decisions(now time.Time, nodes []Node) []TrafficDecision {
    t.mu.RLock()
    flows := append([]TrafficFlowObservation(nil), t.flows...)
    endpoints := append([]ManagedEndpoint(nil), t.endpoints...)
    controllers := make(map[string]*TrafficPathController, len(t.controllers))
    for serviceID, controller := range t.controllers { controllers[serviceID] = controller }
    t.mu.RUnlock()

    services := make(map[string]struct{})
    for _, f := range flows { if f.ServiceID != "" { services[f.ServiceID] = struct{}{} } }
    for _, e := range endpoints {
        if e.ServiceID != "" && (e.ExpiresAt.IsZero() || now.Before(e.ExpiresAt)) { services[e.ServiceID] = struct{}{} }
    }
    byService := make(map[string][]TrafficPathObservation, len(services))
    for serviceID := range services {
        p, ok := trafficPolicyByID(serviceID)
        if !ok { continue }
        qualityByPath := make(map[string]TrafficQualityObservation)
        for _, q := range t.QualitySnapshot(serviceID, now) { qualityByPath[q.PathID] = q }
        for _, n := range nodes {
            q, measured := qualityByPath[n.ID]
            if !n.Healthy && !measured { continue }
            ...
            if !nodeHasService(n, serviceID) && !measured { continue }
            if !usableObservation(o, p) { continue }
    ...
    for serviceID, observations := range byService {
        p, ok := trafficPolicyByID(serviceID)
        if !ok || len(observations) == 0 { continue }
        c := controllers[serviceID]
        if c == nil {
            c = &TrafficPathController{}
            t.mu.Lock()
            if existing := t.controllers[serviceID]; existing != nil { c = existing } else { t.controllers[serviceID] = c }
            t.mu.Unlock()
        }
        d, ok := c.Decide(observations, p, now)
        if !ok { continue }
        t.mu.Lock()
...
func flowListenerConfigFromEnv() (FlowListenerConfig, bool) {
    ...
    if address == "" || exportersRaw == "" { return FlowListenerConfig{}, false }
    exporters := make([]string, 0, 16)
    for _, item := range strings.Split(exportersRaw, ",") {
        item = strings.TrimSpace(item)
        if item != "" { exporters = append(exporters, item); if len(exporters) == 256 { break } }
    }
