...
func (a *App) authorize(r *http.Request, permission string) error {
	rc:=requestInfo(r); if a.db==nil||rc.PrincipalID==""||rc.TenantID==""{return errors.New("authorization context unavailable")}; var allowed bool
	err:=a.db.QueryRow(r.Context(),`select exists(select 1 from principal_roles pr join roles ro on ro.id=pr.role_id and ro.tenant_id=$1::uuid join role_permissions rp on rp.role_id=ro.id join permissions p on p.id=rp.permission_id where pr.principal_id=$2::uuid and p.key=$3)`,rc.TenantID,rc.PrincipalID,permission).Scan(&allowed)
