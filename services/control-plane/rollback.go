...
func (a *App) rollbackJob(w http.ResponseWriter, r *http.Request) {
	...
	if dec.Decode(&q) != nil || strings.TrimSpace(q.ApprovalID) == "" { jsonResponse(w, 400, map[string]string{"error": "rollback_approval_required"}); return }
	var extra any
	if err := dec.Decode(&extra); err != io.EOF { jsonResponse(w, 400, map[string]string{"error": "multiple_json_values"}); return }
	rc := requestInfo(r)
	tx, err := a.db.Begin(r.Context())
	if err != nil { jsonResponse(w, 500, map[string]string{"error": "rollback_begin_failed"}); return }
	...
	if errors.Is(err, pgx.ErrNoRows) { jsonResponse(w, 404, map[string]string{"error": "job_or_approval_not_found"}); return }
	if err != nil { jsonResponse(w, 500, map[string]string{"error": "rollback_query_failed"}); return }
	if jobStatus != "succeeded" { jsonResponse(w, 409, map[string]string{"error": "job_not_successful"}); return }
	if approvalStatus != "approved" || approvalAction != "job.rollback" || approvalResource != id { jsonResponse(w, 409, map[string]string{"error": "rollback_approval_not_bound"}); return }
	if len(rollbackPayload) == 0 { rollbackPayload = []byte(`{}`) }
	if payloadHash == "" || payloadHash != requestBodyHash(json.RawMessage(rollbackPayload)) { jsonResponse(w, 409, map[string]string{"error": "rollback_payload_mismatch"}); return }
	...
	res, err := tx.Exec(r.Context(), `
		...
		where id=$1::uuid and tenant_id=$5::uuid and status='succeeded'
