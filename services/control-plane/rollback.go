...
// A separate approval is mandatory so rollback remains inside the same approval-first boundary.
func (a *App) rollbackJob(w http.ResponseWriter,r *http.Request){
    ...
    var q rollbackRequest;if json.NewDecoder(r.Body).Decode(&q)!=nil||strings.TrimSpace(q.ApprovalID)==""{jsonResponse(w,400,map[string]string{"error":"rollback_approval_required"});return}
    ...
    var jobStatus,approvalStatus,action string;var rollbackPayload []byte
    err=tx.QueryRow(r.Context(),`select j.status,a.status,j.execution_action,j.rollback_payload::text from durable_jobs j join change_approvals a on a.id=$3::uuid where j.id=$1::uuid and j.tenant_id=$2::uuid`,id,rc.TenantID,q.ApprovalID).Scan(&jobStatus,&approvalStatus,&action,&rollbackPayload)
    ...
    if jobStatus!="succeeded"{jsonResponse(w,409,map[string]string{"error":"job_not_successful"});return};if approvalStatus!="approved"{jsonResponse(w,409,map[string]string{"error":"rollback_approval_not_approved"});return}
    if len(rollbackPayload)==0{rollbackPayload=[]byte(`{}`)}
    _,err=tx.Exec(r.Context(),`update durable_jobs set status='queued',attempts=0,max_attempts=3,available_at=now(),locked_at=null,locked_by=null,finished_at=null,last_error='',execution_action=case when execution_action='' then 'rollback' else execution_action||'.rollback' end,approval_id=$2::uuid,correlation_id=$3,updated_at=now() where id=$1::uuid and tenant_id=$4::uuid`,id,q.ApprovalID,rc.CorrelationID,rc.TenantID)
