...
func (c *FlowTemplateCache) Put(key FlowExporterKey, t FlowTemplate) error { if c==nil{return errors.New("flow template cache is required")}; if t.ID<256||len(t.Fields)==0||len(t.Fields)>64{return errors.New("invalid flow template")}; c.mu.Lock();defer c.mu.Unlock();if c.templates==nil{c.templates=make(map[FlowExporterKey]map[uint16]FlowTemplate)};m:=c.templates[key];if m==nil{m=make(map[uint16]FlowTemplate);c.templates[key]=m};if _,exists:=m[t.ID];!exists&&len(m)>=maxTemplatesPerExporter{return errors.New("template cache limit reached")};t.Fields=append([]FlowTemplateField(nil),t.Fields...);m[t.ID]=t;return nil }
func (c *FlowTemplateCache) Get(key FlowExporterKey,id uint16)(FlowTemplate,bool){if c==nil{return FlowTemplate{},false};c.mu.RLock();defer c.mu.RUnlock();m:=c.templates[key];if m==nil{return FlowTemplate{},false};t,ok:=m[id];if !ok{return FlowTemplate{},false};t.Fields=append([]FlowTemplateField(nil),t.Fields...);return t,true}
func readU16(b []byte,off *int)(uint16,error){if off==nil||*off<0||*off+2>len(b){return 0,errFlowPacketTruncated};v:=binary.BigEndian.Uint16(b[*off:*off+2]);*off+=2;return v,nil}
...
func(c *FlowTelemetryCollector)Collect(ctx context.Context,packet []byte)([]FlowRecord,error){if c==nil{return nil,errors.New("flow telemetry collector is required")};if ctx==nil{return nil,errors.New("context is required")};if err:=ctx.Err();err!=nil{return nil,err};if len(packet)<2{return nil,errFlowPacketTruncated};switch binary.BigEndian.Uint16(packet[:2]){case 5:return DecodeNetFlowV5(packet,"unknown");case 9:return nil,errors.New("netflow v9 requires exporter identity and template-aware decoding");case 10:return nil,errors.New("ipfix requires exporter identity and template-aware decoding");default:return nil,errFlowUnsupportedVersion}}
