...
func (a *App) createApproval(w http.ResponseWriter,r *http.Request){
    ...
    rc:=requestInfo(r);hash:=requestScopedHash(r,req);var id,status string;var expires time.Time
    err:=a.db.QueryRow(r.Context(),`insert into change_approvals(tenant_id,requested_by,action,resource,request_hash,status,expires_at) values($1,$2,$3,$4,$5,'pending',now()+make_interval(secs=>$6)) on conflict(request_hash) do update set updated_at=now() where change_approvals.tenant_id=$1 returning id::text,status,expires_at`,rc.TenantID,rc.PrincipalID,req.Action,req.Resource,hash,req.ExpiresIn).Scan(&id,&status,&expires);if err!=nil{jsonResponse(w,500,map[string]string{"error":"approval_persist_failed"});return}
