...
func TestDispatchNetFlowV9TemplateAndData(t *testing.T) {
	...
	binary.BigEndian.PutUint64(packet[off+12:off+20], 1234)

	got, err := DispatchFlowPacket(packet, "192.0.2.1", cache)
	if err != nil { t.Fatalf("dispatch: %v", err) }
	if got.Metadata.Version != 9 || got.Metadata.ObservationDomain != 42 || got.Metadata.Sequence != 7 { t.Fatalf("bad metadata: %+v", got.Metadata) }
	if got.Templates != 1 || len(got.Records) != 1 { t.Fatalf("templates=%d records=%d", got.Templates, len(got.Records)) }
	if got.Records[0].SourceIP != "10.0.0.1" || got.Records[0].DestinationIP != "8.8.8.8" || got.Records[0].Bytes != 1234 { t.Fatalf("bad record: %+v", got.Records[0]) }
}
...
func TestDispatchIPFIXEnterpriseTemplate(t *testing.T) {
	...
	if err != nil { t.Fatalf("dispatch: %v", err) }
	if got.Metadata.Version != 10 || got.Metadata.ObservationDomain != 77 || got.Metadata.Sequence != 9 { t.Fatalf("bad metadata: %+v", got.Metadata) }
	if got.Templates != 1 || len(got.Records) != 1 { t.Fatalf("templates=%d records=%d", got.Templates, len(got.Records)) }
	if got.Records[0].SourceIP != "10.0.0.2" || got.Records[0].DestinationIP != "10.0.0.3" { t.Fatalf("bad record: %+v", got.Records[0]) }
}
