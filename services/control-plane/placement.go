...
func validNode(n Node) bool { return n.ID != "" && n.Provider != "" && n.CPUPercent >= 0 && n.CPUPercent <= 100 && n.RAMPercent >= 0 && n.RAMPercent <= 100 && n.SSDPercent >= 0 && n.SSDPercent <= 100 && n.HDDPercent >= 0 && n.HDDPercent <= 100 && n.NetMbps >= 0 && n.LatencyMs >= 0 && n.PacketLoss >= 0 && n.PacketLoss <= 100 }
func serviceExists(catalog []Service, serviceID string) bool { for _, s := range catalog { if s.ID == serviceID { return true } }; return false }
func nodeScore(n Node, req PlacementRequest) float64 { if !n.Healthy || !nodeHasService(n, req.ServiceID) { return -1 }; score := 100.0 - n.CPUPercent*.20 - n.RAMPercent*.20 - n.SSDPercent*.10 - n.HDDPercent*.05 - n.LatencyMs*.50 - n.PacketLoss*2; if req.Region != "" && strings.EqualFold(req.Region,n.Region) { score += 20 }; if req.Provider != "" && strings.EqualFold(req.Provider,n.Provider) { score += 10 }; return score }
