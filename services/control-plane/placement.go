...
func nodeScore(n Node, req PlacementRequest) float64 {
	...
	if req.Region != "" && strings.EqualFold(req.Region, n.Region) { score += 20 }
	if req.Provider != "" && strings.EqualFold(req.Provider, n.Provider) { score += 10 }
	return score
}
...
func (a *App) placement(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range nodes { if nodeScore(n, req) >= 0 { candidates = append(candidates, n) } }
	...
	jsonResponse(w, http.StatusOK, map[string]any{
		"status":"placement_ready", "service_id":req.ServiceID, "node_id":best.ID,
		"provider":best.Provider, "region":best.Region, "score":nodeScore(best, req),
		"candidates":candidates, "execution":"approval_required",
	})
}
