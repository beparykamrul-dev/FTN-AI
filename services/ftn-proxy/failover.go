...
func SelectFailover(policy FailoverPolicy,ranked []Upstream,failedID string,attempt int)(Upstream,bool){max:=policy.MaxAttempts;if max<=0{max=2};if attempt<0||attempt>=max{return Upstream{},false};failedID=strings.TrimSpace(failedID);seen:=make(map[string]struct{},len(ranked));for _,u:=range ranked{id:=strings.TrimSpace(u.ID);if id==""||id==failedID{continue};if _,ok:=seen[id];ok{continue};seen[id]=struct{}{};if policy.RequireHealthy&&!u.Healthy{continue};if policy.RequireSecure&&!u.Secure{continue};u.ID=id;u.Endpoint=strings.TrimSpace(u.Endpoint);if u.Endpoint==""{continue};return u,true};return Upstream{},false}
