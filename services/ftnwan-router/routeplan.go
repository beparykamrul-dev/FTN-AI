...
func (p RoutePlanner) PlanRouteChange(ctx context.Context, r Route) (string, error) {
	...
	payload, err := json.Marshal(r); if err != nil { return "", fmt.Errorf("marshal route: %w", err) }
	h := sha256.Sum256(payload)
	plan := RoutePlan{ID: "route-" + hex.EncodeToString(h[:8]), Route: r}
	out, err := json.Marshal(plan); if err != nil { return "", err }
	return string(out), nil
}

func validateRoute(r Route) error {
	if strings.TrimSpace(r.Prefix) == "" { return fmt.Errorf("route prefix is required") }
	prefix, err := netip.ParsePrefix(r.Prefix); if err != nil { return fmt.Errorf("invalid route prefix: %w", err) }
	if r.NextHop != "" {
		hop, err := netip.ParseAddr(r.NextHop); if err != nil { return fmt.Errorf("invalid next hop: %w", err) }
...
func validatePlan(plan RoutePlan) error {
	...
	payload, err := json.Marshal(plan.Route); if err != nil { return fmt.Errorf("marshal route: %w", err) }
	h := sha256.Sum256(payload)
	if plan.ID != "route-"+hex.EncodeToString(h[:8]) { return fmt.Errorf("route plan integrity check failed") }
	return nil
}
...
func decodePlan(planJSON string) (RoutePlan, error) {
	...
	var extra any
	if err := dec.Decode(&extra); err != io.EOF { if err == nil { return RoutePlan{}, fmt.Errorf("multiple route plan values") }; return RoutePlan{}, fmt.Errorf("invalid trailing route plan data: %w", err) }
...
func (p RoutePlanner) ApplyApprovedPlan(ctx context.Context, planJSON string) error {
	if p.Plane == nil { return fmt.Errorf("dataplane is required") }
	plan, err := decodePlan(planJSON); if err != nil { return err }
