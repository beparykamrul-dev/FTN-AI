...
type SessionAdmission interface { AuthFailure(net.IP,time.Time) bool; AuthSuccess(net.IP); }

func ServeSession(ctx context.Context, conn net.Conn, cfg SessionConfig, auth Authenticator, delivery Delivery, admission ...SessionAdmission) error {
	...
	var throttle SessionAdmission; if len(admission)>0 { throttle=admission[0] }
	var remoteIP net.IP; if host,_,e:=net.SplitHostPort(conn.RemoteAddr().String()); e==nil { remoteIP=net.ParseIP(host) }
	...
	for { select{case <-ctx.Done():return ctx.Err();default:}
		...
		case "AUTH":
			...
			id,e:=(AuthExchange{Auth:auth}).AuthenticatePLAIN(ctx,encoded);if e!=nil {if throttle!=nil&&remoteIP!=nil {throttle.AuthFailure(remoteIP,time.Now())};if e2:=write(535,"Authentication failed");e2!=nil{return e2};continue};authenticated=true;identityID=id;if throttle!=nil&&remoteIP!=nil {throttle.AuthSuccess(remoteIP)};if e:=write(235,"Authentication successful");e!=nil{return e}
		case "MAIL":
