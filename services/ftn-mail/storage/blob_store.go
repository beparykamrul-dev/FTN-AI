...
func safeBlobPath(root, key string) (string, error) {
	if root == "" || key == "" || filepath.IsAbs(key) {
	...
	path, err := filepath.Abs(filepath.Join(rootClean, clean))
	if err != nil {
		return "", err
	}
...
func (s *BlobStore) Put(ctx context.Context, key string, plaintext []byte) error {
	...
	if s == nil || s.AEAD == nil {
		return errors.New("invalid blob store")
	}
	path, err := safeBlobPath(s.Root, key)
	if err != nil {
		return err
	}
...
func (s *BlobStore) Get(ctx context.Context, key string) ([]byte, error) {
	...
	if s == nil || s.AEAD == nil {
		return nil, errors.New("invalid blob store")
	}
	path, err := safeBlobPath(s.Root, key)
	if err != nil {
		return nil, err
	...
	if len(data) < s.AEAD.NonceSize() {
		return nil, errors.New("corrupt mail blob")
	}
