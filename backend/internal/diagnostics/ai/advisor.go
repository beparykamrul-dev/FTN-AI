...
func (Advisor) Advise(d model.Diagnosis) Recommendation {
	...
	evidence:=make([]string,0,len(d.Evidence)); seen:=map[string]struct{}{}; for _,e:=range d.Evidence{e=strings.TrimSpace(e);if e==""||len(e)>2048{continue};if _,ok:=seen[e];ok{continue};seen[e]=struct{}{};evidence=append(evidence,e)}
	cause:=strings.TrimSpace(d.Cause); if len(cause)>4096{cause=cause[:4096]}; if cause==""{return Recommendation{Summary:"Insufficient evidence for a reliable root-cause recommendation.",Confidence:confidence,Evidence:evidence,ApprovalRequired:true}}
