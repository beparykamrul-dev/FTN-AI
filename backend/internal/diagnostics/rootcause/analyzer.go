...
func (Analyzer) Find(evidence []model.Evidence, dependencies []model.Dependency) []Candidate {
	...
	for _, item := range evidence {
		if strings.TrimSpace(item.Summary) == "" || strings.TrimSpace(item.ID) == "" {
			continue
		}
...
func (Analyzer) Validate(candidates []Candidate, evidence []model.Evidence) model.Diagnosis {
	...
	for _, e := range evidence {
		if e.ID != "" {
			validEvidence[e.ID] = struct{}{}
		}
	}
	for _, best := range candidates {
		if strings.TrimSpace(best.Cause) == "" || math.IsNaN(best.Confidence) || math.IsInf(best.Confidence, 0) || best.Confidence < 0 || best.Confidence > 1 {
			continue
		}
		bound := make([]string, 0, len(best.Evidence))
		for _, id := range best.Evidence {
			if _, ok := validEvidence[id]; ok {
				bound = append(bound, id)
			}
		}
		if len(best.Evidence) > 0 && len(bound) == 0 {
			continue
		}
