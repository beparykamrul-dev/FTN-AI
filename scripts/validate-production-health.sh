...
check_stack(){
  local compose="$1" elapsed=0 service state health exit_code restart actual
  local config_json
  config_json="$(docker compose --env-file "$ENV_FILE" -f "$compose" config --format json)"
  ...
  while (( elapsed <= TIMEOUT_SECONDS )); do
    local ps
    ps="$(docker compose --env-file "$ENV_FILE" -f "$compose" ps -a --format '{{.Service}}|{{.State}}|{{.Health}}|{{.ExitCode}}' 2>/dev/null || true)"
    local all_ok=1 missing=''
    for service in "${expected[@]}"; do
      actual="$(printf '%s\n' "$ps" | awk -F'|' -v s="$service" '$1==s {print; exit}')"
      if [ -z "$actual" ]; then
        all_ok=0; missing+=" $service"; continue
      fi
      ...
      if [ "$restart" = "no" ]; then
        if [ "$state" != "exited" ] || [ "$exit_code" != "0" ]; then
          all_ok=0; log "${compose#$ROOT_DIR/}: $service one-shot state=$state exit_code=$exit_code"
        fi
        continue
      fi
      if [ "$state" != "running" ]; then
        all_ok=0; log "${compose#$ROOT_DIR/}: $service state=$state"; continue
      fi
      if [ -n "$health" ] && [ "$health" != "healthy" ]; then
        all_ok=0; log "${compose#$ROOT_DIR/}: $service health=$health"
      fi
    done
  ...
  docker compose --env-file "$ENV_FILE" -f "$compose" ps >&2 || true
  docker compose --env-file "$ENV_FILE" -f "$compose" logs --tail=100 >&2 || true
