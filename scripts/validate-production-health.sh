...
check_stack(){
  local compose="$1"
  local elapsed=0
  local expected actual service state health
  mapfile -t expected < <(docker compose --profile '*' --env-file "$ENV_FILE" -f "$compose" config --services)
  ((${#expected[@]})) || fail "No services resolved from $compose"

  while (( elapsed <= TIMEOUT_SECONDS )); do
    local ps
    ps="$(docker compose --profile '*' --env-file "$ENV_FILE" -f "$compose" ps -a --format '{{.Service}}|{{.State}}|{{.Health}}' 2>/dev/null || true)"
    local all_ok=1
    local missing=''
    for service in "${expected[@]}"; do
      ...
      IFS='|' read -r _service state health <<< "$actual"
      # One-shot services (restart: no) are validated by their own command and
      # are not required to remain running after successful completion.
      restart="$(docker compose --profile '*' --env-file "$ENV_FILE" -f "$compose" config --format json | python3 -c 'import json,sys; d=json.load(sys.stdin); s=sys.argv[1]; print(d.get("services",{}).get(s,{}).get("restart", ""))' "$service")"
      if [ "$restart" = "no" ]; then
        [[ "$state" =~ ^(exited|created)$ ]] || { all_ok=0; log "${compose#$ROOT_DIR/}: $service unexpected state=$state"; }
        continue
      fi
