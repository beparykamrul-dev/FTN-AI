...
for service,spec in x.get("services",{}).items():
    for p in spec.get("ports",[]) or []:
        ...
        published=str(p["published"]); protocol=str(p.get("protocol","tcp"))
...
def wildcard(ip): return ip in {'0.0.0.0','::','*'}

rows=[]
with open(sys.argv[1],encoding='utf-8') as fh:
    for line in fh:
        ip,published,proto,service,compose=line.rstrip('\n').split('\t',4)
        try: lo,hi=interval(published)
        except ValueError as exc: raise SystemExit(f'invalid published port range {published!r} in {compose}: {exc}')
        if not (1 <= lo <= hi <= 65535): raise SystemExit(f'invalid published port range {published!r} in {compose}')
...
for i,a in enumerate(rows):
    ...
    for b in rows[i+1:]:
        ...
        if aproto != bproto or ahi < blo or bhi < alo: continue
        if not (aip == bip or wildcard(aip) or wildcard(bip)): continue
        raise SystemExit('duplicate production host binding: '
